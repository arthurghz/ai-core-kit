...
>    `specs/ARCHITECTURE.md` (the boundary + data flow), drawing invariants from
>    `specs/DOMAIN.md`.
> 2. **Populate the machine-readable interface** the archetype ships — e.g.
>    `openapi/openapi.yaml` — with those operations + schemas (context, not app code).
...
> front**. For non-API projects (`api_first` false), author in the normal doc order below.
