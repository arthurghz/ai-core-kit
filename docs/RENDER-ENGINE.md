...
`.when` is expressed two ways — **path-segment guards** and **`render.map.yaml` glob
guards** — and BOTH mechanisms are kept; the following four rules formalize how they
combine (there is no "drop one" — each expresses inclusion the other cannot cleanly):

1. **Path-segment guards evaluated FIRST.** If a file's output path contains one or more
   directory segments of the form `_when.<bool.path>/`, ALL of them must be truthy for
   inclusion; the segments are stripped from the output path. Example:
   `templates/archetypes/fullstack/_when.design_system.install/design-system/...`
   renders only when `design_system.install == true`. A false path-segment
   **short-circuits to omit** before the map is consulted (this is what "path-segment
   wins" means: it is evaluated first and a false segment never reaches `render.map.yaml`).
2. **`render.map.yaml` glob guards then apply** by glob, matched against the **post-strip**
   output path, for anything not naturally expressed as a path segment (cross-cutting file
   globs such as `.mcp.json` and the gate hook). A file that has BOTH a path-segment guard
   AND a `render.map.yaml` `when` is included only if **BOTH are truthy** (logical AND; no
   override in either direction).
3. **`requires_archetype` is an assertion, not a selector.** If a `render.map.yaml` rule's
   glob matches AND its `when` resolves truthy AND `managed.archetype != requires_archetype`,
   the render **ABORTS loudly** (an authoring bug). It NEVER silently omits.
4. **Absent `managed:` key** referenced by any `when` (path-segment or map) ⟹ false ⟹ omit.

The sidecar `render.map.yaml` at each archetype root declares the per-glob guards. Shape:
...
- **fullstack:** schema **requires** `design_system`; with `install: true` the renderer
  ...
  source-available content; manifest findings 21/45). `tokens` (e.g. `color_brand`)
  flow in as `${design_system.tokens.color_brand}`.
...
rendered_files:
  - path: .claude/settings.json
    managed_block: "json:hooks,env"  # JSON key-set ownership (not a comment region)
  - path: CLAUDE.md
    managed_block: "ack:managed"     # text file: renderer owns a delimited comment region
...
For files the renderer shares with the human it owns **only** a bounded slice and
rewrites *only* that slice. For **text** files (`CLAUDE.md`) the slice is a delimited
comment region; for **JSON** files (`settings.json`, `.mcp.json`) the slice is a declared
**set of keys** (see the JSON key-set rules below) — there are no comment markers in JSON:

```jsonc
// .claude/settings.json — ack owns ONLY the keys `hooks` and `env`
// (no comment markers; ownership is by key set: managed_block = "json:hooks,env")
{
  "permissions": { "allow": ["Bash(uv run *)"] },   // ← HUMAN-OWNED, never touched
  "env": { "MY_TOKEN": "x",                          // ← human key, never touched
           "CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS": "1" },  // ← ack-owned nested key only
...
verbatim. First run, if a **text** target (`CLAUDE.md`, other Markdown/text) exists
without markers, the renderer *appends* the managed block at end and never
reorders/strips human content. Comment-marker blocks are reserved for text files ONLY.

**JSON targets (`settings.json`, `.mcp.json`) use JSON key-set ownership, NOT comment
markers** (JSON has no comments to delimit a region). For these, `managed_block` is NOT a
comment delimiter — it is the JSON ownership descriptor `json:<key1>,<key2>`. The renderer
**parses** the file, **replaces only the declared managed keys**, and **re-serializes**,
never reordering or stripping human-owned keys. The authoritative managed key sets are:

- **`.claude/settings.json`** — managed keys = `hooks`, `env` (`managed_block: "json:hooks,env"`):
  - `hooks` — owned only when `features.sdd_gate: true` (the `PreToolUse` matcher entry
    for the contract gate). When `sdd_gate: false`, `hooks` leaves the managed set and the
    renderer deletes only the ack-inserted matcher entry it previously wrote.
  - `env` — owned only when `features.agent_teams: true`, and **within** `env` ack owns
    ONLY the single key `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` (nested-key ownership; all
    other `env` keys are user territory and are never touched).
  - `permissions` and every other top-level key are **HUMAN-OWNED**, never written.
- **`.mcp.json`** — managed keys = `mcpServers` (`managed_block: "json:mcpServers"`), and
  **within** `mcpServers` ack owns ONLY the server entries it renders, keyed by the
  ack-prefixed name `ack-*`. A user-added server (non-`ack-` key) is never touched; an ack
  server is added only if absent, with a re-approval warning (finding 41).
...
Template (`render_json` path; ack owns the `hooks` key by JSON key-set ownership — no
comment markers):
```jsonc
{
  "hooks": { "PreToolUse": [ {
...
managed:
  rendered_files:
    - { path: .claude/settings.json, managed_block: "json:hooks,env" }
...
- User then flips `features.mcp` → true and re-runs: hash changes ⇒ renderer adds
  `.mcp.json` (now `when` true), writing only the ack-owned `mcpServers.ack-*` entries;
  it updates the `CLAUDE.md` `ack:managed` comment region and the `settings.json`
  managed keys (`hooks`,`env`) by key-set, leaves the human `permissions` block, any
  non-`ack-` MCP server, and the house-style paragraph byte-identical, and appends
  `.mcp.json` (`managed_block: "json:mcpServers"`) to `rendered_files[]`.
...
2. **Conditionals live in the manifest** (path-segment `_when.*` + `render.map.yaml`),
   ...
   Path-segment guards are evaluated FIRST and a false one short-circuits to omit; a file
   carrying both a path-segment guard and a `render.map.yaml` `when` is included only if
   BOTH are truthy (logical AND). `requires_archetype` is an abort-assertion, never a
   selector.
