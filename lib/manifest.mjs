...
export function assembleManaged(answers, opts = {}) {
  ...
  // AUTH is borne by BOTH the web archetypes: saas AND fullstack ALWAYS ship Clerk
  // (USER DECISION — "saas e fullstack always should have CLERK"). hosting + billing
  // stay saas-only. auth is schema-optional everywhere + the enum includes clerk, so
  // no schema change is needed; we just emit it for fullstack too, defaulting clerk.
  const isAuthBearing = archetype === "saas" || archetype === "fullstack";
  const rawAnswer = (k) =>
  ...
  if (isAuthBearing || has("auth_provider") || rawAnswer("auth_provider") !== undefined) {
