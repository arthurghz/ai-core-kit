...
export function assembleManaged(answers, opts = {}) {
  ...
  // features.iac is OPTIONAL (default false); emit it only when the interview
  // (Phase D feat_iac) actually fired OR a raw answer supplied it (sidecar
  // pattern), so existing manifests stay byte-stable and the key is never written
  // as a redundant default. Absent => treated as false (no iac block).
  if (has("feat_iac")) {
    managed.features.iac = boolOr(val("feat_iac"), false);
  } else if (answers && Object.prototype.hasOwnProperty.call(answers, "feat_iac")) {
    managed.features.iac = boolOr(answers.feat_iac, false);
  }
