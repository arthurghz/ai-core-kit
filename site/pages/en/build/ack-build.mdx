...
```bash
# See the resolved plan without authoring anything (safe first move):
/ack-build --dry-run

# Resume the sweep from the next unbuilt phase, stopping at every gate:
/ack-build --from P4

# Build exactly one phase (its depends_on must already be done):
/ack-build --phase P5

# Re-author a completed phase from scratch (e.g. after editing its deliverables):
/ack-build --phase P3 --rebuild

# Unattended CI sweep — never pause at a gate (gates with failing tests still stop):
/ack-build --no-stop
```

The flags compose: `--from P4 --no-stop` runs P4 onward without pausing,
`--phase P5 --rebuild` forces a single phase. `--dry-run` overrides everything
else — it only prints the plan.
...
- [Bootstrap config (ack.bootstrap.yaml)](/build/bootstrap-config) — the data
  this command reads: the eight-phase plan, roster, models, budgets, and
  acceptance tests.
- [Offline Cost Telemetry](/features/cost-telemetry) — how STEP 5's cumulative
  cost is computed, and why it is offline-only.
- [The META vs CHILD boundary](/concepts/two-layer-model) — why `/ack-build`
  (builds the kit) is the inverse of [`/ack-init`](/reference/commands)
  (renders a child).
- [Roadmap](/roadmap) — the honest shipped / partial / roadmap status of each
  phase this command drives.
