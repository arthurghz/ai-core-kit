...
Every Claude Code primitive is markdown with YAML frontmatter. The rules below are the canonical standard ([CONVENTIONS.md](https://github.com/arthurghz/ai-core-kit/blob/main/docs/CONVENTIONS.md)) and are **mechanically enforced** (see §3).
...
- [The META vs CHILD boundary](/concepts/two-layer-model) — the full treatment of the §1 rule every contribution must respect.
- [Skills Catalog](/features/skills-catalog) — where `skill-creator` and `skill-validator` live, plus how a CHILD-payload skill gets wired into a fork.
- [Commands Reference](/reference/commands) and [Agents Reference](/reference/agents) — the conventions for the other two primitive types.
- [License Ledger](/reference/licensing) — the vendoring checklist and the four reference repos' license posture.
