...
the pricing map, budgets, the DORA module, the Prometheus exporter, and the
docker-compose stack with its three dashboards. The cost attribution model is
detailed in [Offline Cost Telemetry](/features/cost-telemetry).
...
|---|---|---|---|
| `aggregate.py` | telemetry | META | Offline **cost + token** aggregator — reads transcripts, multiplies token counts by the versioned pricing map, attributes by `model`/`feature`/`agent`/`session`/`day`, and compares totals to advisory budgets. |
| `pricing.json` | telemetry | META | Versioned `model → USD/MTok` map, `unknown_model_policy=error`. |
| `dora.py` | telemetry | META | The DORA **four keys** from local git history (+ optional `gh`), with a self-test. Text / JSON / Prometheus output. |
| `ack-cost-exporter` | telemetry | META | Thin Prometheus wrapper that **imports** `aggregate.py` (no re-implementation) and exposes cost/token gauges on `/metrics`. |
| `observability-stack` | telemetry | META | Prometheus + Grafana + exporter docker-compose stack with three dashboards. |
...
is exact** (derived from git, not the transcript). Two CHILD payload skills sit
on top of the cost engine (both MIT):
