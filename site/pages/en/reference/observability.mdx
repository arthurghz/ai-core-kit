...
description: Every telemetry and observability primitive ai-core-kit ships — the offline aggregate.py cost-and-token aggregator, the versioned pricing.json map, advisory budgets, the DORA four-keys module (dora.py), the self-contained report.py and the interactive dashboard.py, the Prometheus exporter, and the Prometheus + Grafana docker-compose stack with the ack-cost, ack-ai-usage and ack-dora dashboards.
---
...
## Tier 0 — `dashboard.py` (interactive HTML cost dashboard)
...
python3 telemetry/dashboard.py --serve --watch 5           # local live view, recompute 5s
```

Like `report.py`, it **imports** `aggregate.py` and `dora.py` — it is a *view*,
