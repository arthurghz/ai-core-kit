...
- [Orquestrador /ack-build](/build/ack-build) — o comando que lê, valida e
  conduz esta config fase a fase.
- [Manifesto e Entrevista](/concepts/manifest-and-interview) — o análogo da
  camada CHILD deste arquivo: um schema congelado, uma instância validada
  (`project.manifest.yaml`), um consumidor (`/ack-init`).
- [Telemetria de Custo Offline](/features/cost-telemetry) — o agregador da P6
  que esta config agenda, e por que o custo é apenas offline.
- [Roadmap](/roadmap) — o status entregue / parcial / roadmap que espelha os
  flags `done` em `phases[]`.
