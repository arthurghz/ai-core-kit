...
rules:
  ...
  - glob: "**/design-system/**"
    ...
    requires_archetype: fullstack      # ASSERTION, not a selector
```
...
3. **`requires_archetype` é uma asserção, não um seletor.** Se o glob de uma regra casa e
   seu `when` é verdadeiro mas `managed.archetype` não é igual a `requires_archetype`, o
   render **aborta ruidosamente** (um bug de autoria). Ele nunca omite silenciosamente.
4. **Uma chave `managed:` ausente referenciada por qualquer `when` avalia como falso →
   omite.** É assim que *o design system renderiza apenas para fullstack* sai de graça: o
   schema proíbe `design_system` sob `backend-api`, então a busca está ausente → falso →
   omitido, sem nenhum tratamento especial.
