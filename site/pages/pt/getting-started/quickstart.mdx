...
Quando as specs chegam, estes são os comandos do dia a dia — lista completa no
[**guia de Comandos**](/commands):

| Comando | O que faz |
|---|---|
| `/ack-build` | Constrói a próxima fatia das specs — contrato primeiro, multi-agente |
| `/ack-review` | Revisa sua mudança com um time de agentes antes do PR |
| `/ack-tooling` | Lint / tipos / testes / CI para a sua stack |
| `/ack-cost` · `/ack-reports` | Gasto de IA por feature + entrega (DORA) — offline |
| `/ack-list` · `/ack-features` | Inventário de contratos/fatias/skills · janelas de custo por feature |
| `/ack-sync` · `/ack-update` | Puxa (e verifica) novos comandos/agentes/skills do kit |

> **Dirija por times:** `export CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` antes de iniciar —
