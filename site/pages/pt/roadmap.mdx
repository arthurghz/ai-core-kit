...
- [Config de bootstrap (ack.bootstrap.yaml)](/build/bootstrap-config) — a fonte das flags `done` e `gate` que esta página reporta; o DAG de oito fases.
- [Orquestrador /ack-build](/build/ack-build) — o comando que conduz estas fases e para em cada gate.
- [A fronteira META vs CHILD](/concepts/two-layer-model) — por que um *contrato* congelado (P3) não é o mesmo que um *runtime* acabado.
- [Discovery Engine (P7)](/features/discovery) — o loop propõe-mas-não-adota rastreado na seção Roadmap.
