#!/usr/bin/env python3
# =============================================================================
# report.py -- ai-core-kit Tier-0 "Delivery & AI-cost" report  (CHILD layer)
# =============================================================================
# Shipped into a forked project by `/ack-init` when telemetry.enabled: true.
# This is the SAME combiner as the kit's META telemetry/report.py; only the CLI
# defaults are localized -- it resolves pricing.json and the git repo under
# ${CLAUDE_PROJECT_DIR} (via aggregate.child_project_root), so a fork runs it
# with no flags. It still imports the sibling aggregate.py + dora.py engines and
# duplicates none of their math.
#
# WHAT THIS IS
...
def build_parser():
    ...
    # CHILD: resolve pricing.json + the git repo under ${CLAUDE_PROJECT_DIR} (the
    # same convention aggregate.py uses) so a fork runs `report.py` with no flags.
    root = aggregate.child_project_root()
    ...
    ap.add_argument("--repo", default=root,
                    help="git work tree for the DORA side "
                         "(default: ${CLAUDE_PROJECT_DIR}, this project's root)")
